...
class BankApplication:
    ...
    def load_balance(self):
        try:
            with open(f"{self.bank_data_folder}/{self.user_name}_Bank_Data.txt", "r") as file:
                self.temp_balance = float(file.readline())
    ...
    def save_balance(self):
        ...
        with open(f"{self.bank_data_folder}/{self.user_name}_Bank_Data.txt", "w") as file:
            file.write(str(self.temp_balance))
    ...
    def display_balance(self):
        print(f"Current Balance: R{self.temp_balance:.2f}")

    def validate_positive_float_input(self, prompt):
        while True:
            try:
    ...
    def validate_withdrawal_amount(self):
        while True:
            amount = self.validate_positive_float_input("How much would you like to withdraw? ")
    ...
    def deposit(self):
        amount = self.validate_positive_float_input("How much would you like to deposit? ")
        self.temp_balance += amount
        self.log_transaction("Deposit", amount)
        self.display_balance()

    def withdraw(self):
        amount = self.validate_withdrawal_amount()
        self.temp_balance -= amount
